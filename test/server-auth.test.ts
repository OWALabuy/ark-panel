...
test("记忆候选生成与确认要求登录、CSRF 和服务端 hash 校验", async t => {
  const calls: string[] = [], consolidation: MemoryConsolidationApi = { agents() { return ["safe"]; }, async status(recordId) { calls.push(`status:${recordId}`); return { available: true, eligible: true, pending: true }; }, async candidate(recordId) { calls.push(`candidate:${recordId}`); return { batchId: "batch", contentHash: "a".repeat(64) }; }, async getCandidate(batchId) { calls.push(`get:${batchId}`); return { batchId }; }, async confirm(batchId, hash) { if (batchId === "index-failed") throw new Error("MEMORY_INDEX_REFRESH_FAILED"); calls.push(`confirm:${batchId}:${hash}`); return { status: "confirmed" }; } };
  ...
  assert.deepEqual((await (await fetch(`${x.base}/api/v1/sessions/record/memory/status`, { headers: { cookie: cookies } })).json()).data, { available: true, eligible: true, pending: true });
  ...
  assert.deepEqual(calls, ["status:record", "candidate:record", "get:batch", `confirm:batch:${"a".repeat(64)}`]);
});
