...
test("附件上传下载要求登录与 CSRF，并保持 Office 原始字节", async t => {
  ...
  const x = await fixture(t, undefined, undefined, undefined, attachments);
...
test("API rejects unauthenticated requests", async t => { const x = await fixture(t); const r = await fetch(`${x.base}/api/v1/agents`); assert.equal(r.status, 401); });
test("记忆中心要求登录并只委托 agentId 与相对标识", async t => {
  ...
  const x = await fixture(t, undefined, undefined, undefined, undefined, memory);
...
test("记忆候选生成与确认要求登录、CSRF 和服务端 hash 校验", async t => {
  const calls: string[] = [], consolidation: MemoryConsolidationApi = { agents() { return ["safe"]; }, async status(recordId) { calls.push(`status:${recordId}`); return { available: true, eligible: true, pending: true, recovery: "none" }; }, async candidate(recordId, mode) { calls.push(`candidate:${recordId}:${mode}`); return { batchId: "batch", contentHash: "a".repeat(64) }; }, async getCandidate(batchId) { calls.push(`get:${batchId}`); return { batchId }; }, async confirm(batchId, hash) { if (batchId === "index-failed") throw new Error("MEMORY_INDEX_REFRESH_FAILED"); calls.push(`confirm:${batchId}:${hash}`); return { status: "confirmed" }; } };
  const x = await fixture(t, undefined, undefined, undefined, undefined, undefined, consolidation);
  ...
  assert.deepEqual((await (await fetch(`${x.base}/api/v1/sessions/record/memory/status`, { headers: { cookie: cookies } })).json()).data, { available: true, eligible: true, pending: true, recovery: "none" });
  assert.equal((await fetch(`${x.base}/api/v1/sessions/record/memory/candidates`, { method: "POST", headers: auth, body: JSON.stringify({ mode: "restore" }) })).status, 201);
  assert.equal((await fetch(`${x.base}/api/v1/sessions/record/memory/candidates`, { method: "POST", headers: auth, body: JSON.stringify({ mode: "restore", path: "/browser/chosen" }) })).status, 400);
  ...
  assert.deepEqual(calls, ["status:record", "candidate:record:restore", "get:batch", `confirm:batch:${"a".repeat(64)}`]);
});
test("non-mock reads are delegated without connecting a real agent", async t => {
  const reads:ReadApi={async agents(){return [{id:"safe"}]},async sessions(agentId){assert.equal(agentId,"safe");return [{recordId:"record"}]},async projects(agentId){assert.equal(agentId,"safe");return["Project"]},async conversation(recordId){assert.equal(recordId,"record");return {title:"safe"}}}; const x=await fixture(t, undefined,reads);
...
test("active/reset sources are rejected before generation",async t=>{let calls=0;const reads:ReadApi={async agents(){return[]},async sessions(){return[]},async conversation(){return{sourceKind:"active"}}};const generation=fakeGeneration({async create(){calls++;return snapshot()}});const x=await fixture(t, generation,reads);const login=await fetch(`${x.base}/api/v1/auth/login`,{method:"POST",headers:{origin:x.base,"content-type":"application/json"},body:JSON.stringify({username:"owl",password:"correct"})});const body=await login.json()as{data:{csrfToken:string}};const cookies=login.headers.getSetCookie().map(value=>value.split(";",1)[0]).join("; ");const response=await fetch(`${x.base}/api/v1/sessions/active/runs`,{method:"POST",headers:{cookie:cookies,origin:x.base,"x-csrf-token":body.data.csrfToken,"content-type":"application/json"},body:JSON.stringify({message:"must not run"})});assert.equal(response.status,409);assert.equal((await response.json()).error.code,"SOURCE_READ_ONLY");assert.equal(calls,0)});
test("active/reset sources are rejected before compact command dispatch",async t=>{let calls=0;const reads:ReadApi={async agents(){return[]},async sessions(){return[]},async conversation(){return{sourceKind:"reset"}}},commands:CommandApi={async dispatch(){calls++;return{}}};const x=await fixture(t, undefined,reads,commands);const login=await fetch(`${x.base}/api/v1/auth/login`,{method:"POST",headers:{origin:x.base,"content-type":"application/json"},body:JSON.stringify({username:"owl",password:"correct"})});const body=await login.json()as{data:{csrfToken:string}};const cookies=login.headers.getSetCookie().map(value=>value.split(";",1)[0]).join("; ");const response=await fetch(`${x.base}/api/v1/sessions/readonly/command`,{method:"POST",headers:{cookie:cookies,origin:x.base,"x-csrf-token":body.data.csrfToken,"content-type":"application/json"},body:JSON.stringify({command:"compact",args:[],revision:"rev"})});assert.equal(response.status,409);assert.equal((await response.json()).error.code,"SOURCE_READ_ONLY");assert.equal(calls,0)});
...
test("run creation and SSE observation share one durable runId", async t => {
  let createCalls=0;const generation=fakeGeneration({async create(recordId,message,runId,_revision,_attachmentIds,requestOutputs){createCalls++;assert.equal(recordId,"record-1");assert.equal(message,"虚构消息");assert.equal(requestOutputs,true);return snapshot("accepted",runId)},async get(runId){return snapshot("completed",runId)},async subscribe(runId,listener){listener(snapshot("completed",runId));return()=>undefined}});const x = await fixture(t, generation);
  const login=await fetch(`${x.base}/api/v1/auth/login`,{method:"POST",headers:{origin:x.base,"content-type":"application/json"},body:JSON.stringify({username:"owl",password:"correct"})});
  ...
  const invalid=await fetch(`${x.base}/api/v1/sessions/record-1/runs`,{method:"POST",headers:{cookie:cookies,origin:x.base,"x-csrf-token":loginBody.data.csrfToken,"content-type":"application/json"},body:JSON.stringify({message:"虚构消息",requestOutputs:"yes"})});assert.equal(invalid.status,400);assert.equal((await invalid.json()).error.code,"REQUEST_OUTPUTS_INVALID");assert.equal(createCalls,0);
  const created=await fetch(`${x.base}/api/v1/sessions/record-1/runs`,{method:"POST",headers:{cookie:cookies,origin:x.base,"x-csrf-token":loginBody.data.csrfToken,"content-type":"application/json","idempotency-key":testRunId},body:JSON.stringify({message:"虚构消息",requestOutputs:true})});assert.equal(created.status,202);assert.equal((await created.json()).data.runId,testRunId);assert.equal(createCalls,1);
...
test("结构化命令接口受登录和 CSRF 保护并委托命令派发器", async t => {
  const calls: unknown[] = []; const x = await fixture(t, undefined, undefined, { async dispatch(recordId, request) { calls.push([recordId, request]); return { effect: "read" }; } });
...
test("fixed Host policy, logout, request limit, and static symlink boundary", async t => {
  const publicDir=await tempFixture(t,"panel-web-secure-"), outside=join(await tempFixture(t,"panel-outside-"),"secret.txt"); await writeFile(join(publicDir,"index.html"),"ok"); await writeFile(outside,"secret"); await symlink(outside,join(publicDir,"escape.txt"));
  const auth={username:"owl",passwordHash:passwordHash("correct","0011223344556677"),sessionSecret:"test-secret-long-enough"};
  const allowedHosts:string[]=[],publicOrigins:string[]=[]; const server=createPanelServer({auth,publicDir,allowedHosts,publicOrigins,generation:fakeGeneration()}); t.after(async()=>{if(server.listening){server.close();await once(server,"close")}}); server.listen(0,"127.0.0.1"); await once(server,"listening"); const address=server.address(); if(!address||typeof address==="string")throw new Error("no address"); const base=`http://127.0.0.1:${address.port}`;
...
test("失败 run 可通过 SSE 终态发现，未知 run 在写 SSE 头前返回 JSON 404", async t => {
  const failed={...snapshot("failed"),error:{code:"CONTEXT_BUDGET_EXCEEDED",message:"会话历史过长"}};const x=await fixture(t, fakeGeneration({async get(id){return id===testRunId?failed:undefined},async subscribe(_id,listener){listener(failed);return()=>undefined}}));
  const login=await fetch(`${x.base}/api/v1/auth/login`,{method:"POST",headers:{origin:x.base,"content-type":"application/json"},body:JSON.stringify({username:"owl",password:"correct"})});const cookies=login.headers.getSetCookie().map(v=>v.split(";",1)[0]).join("; ");
...
test("abort 接口返回服务端确认的 run 状态", async t => {
  let calls=0;const x=await fixture(t, fakeGeneration({async abortRun(runId){calls++;return snapshot("aborted",runId)}}));
