...
async function fixture(t: import("node:test").TestContext) { const root = await tempFixture(t, "panel-experience-"); return { root, store: new ExperienceStore(root, ["claude"]) }; }

test("settings 使用严格 schema、局部更新和私有原子持久化", async t => {
...
test("并发 settings patch 不丢失不同字段", async t => {
...
test("旧 settings 文件缺少 conversation 时采用默认值并在下次更新时迁移", async t => {
  const { root, store } = await fixture(t);
...
test("损坏或符号链接 settings 不会被静默覆盖", async t => {
  const { root, store } = await fixture(t); await writeFile(join(root, "settings.json"), "{}");
  await assert.rejects(store.settings(), /SETTINGS_CORRUPT/);
  const linkedRoot = await tempFixture(t, "panel-experience-link-"), outside = join(linkedRoot, "outside.json"); await writeFile(outside, "{}");
...
test("头像真实解码后统一裁切为 256x256 WebP，并可删除", async t => {
  const { root, store } = await fixture(t);
...
test("头像拒绝非 allowlist、伪图片、SVG、动图和超大尺寸", async t => {
  const { store } = await fixture(t); const one = await sharp({ create: { width: 1, height: 1, channels: 3, background: "red" } }).png().toBuffer();
...
test("头像目录若被替换为符号链接则拒绝读写删除", async t => {
  const { root, store } = await fixture(t), outside = await tempFixture(t, "panel-avatar-outside-"); await symlink(outside, join(root, "avatars"));
...
test("读取头像前拒绝超大或非普通持久文件", async t => {
  const { root, store } = await fixture(t); await mkdir(join(root, "avatars"));
...
test("同毫秒并发头像写入使用互不冲突的临时文件", async t => {
  const { store } = await fixture(t); const input = await sharp({ create: { width: 32, height: 32, channels: 3, background: "blue" } }).png().toBuffer();
