...
test("settings 使用严格 schema、局部更新和私有原子持久化", async () => {
  ...
  assert.deepEqual(await store.settings(), { version: 1, appearance: { theme: "system", accent: "default" } });
  assert.deepEqual(await store.patchSettings(validateSettingsPatch({ appearance: { theme: "dark" } })), { version: 1, appearance: { theme: "dark", accent: "default" } });
  assert.deepEqual(await store.patchSettings(validateSettingsPatch({ appearance: { accent: "cyan" } })), { version: 1, appearance: { theme: "dark", accent: "cyan" } });
...
test("并发 settings patch 不丢失不同字段", async () => {
  ...
  assert.deepEqual(await store.settings(), { version: 1, appearance: { theme: "gruvbox-dark-medium", accent: "yellow" } });
});
