...
test("frontend renders untrusted metadata with DOM text nodes", async () => {
  ...
  assert.match(source,/saveDraft\("",sourceSession\)/);
  assert.match(source,/sourceSession=activeSession,sourceRevision=activeRevision/);
  assert.match(source,/encodeURIComponent\(sourceSession\)\}\/messages/);
  assert.match(source,/if\(activeSession===sourceSession\)\{textarea\.value="";await openSession\(sourceSession\)\}/);
