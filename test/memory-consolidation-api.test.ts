...
test("eligible 会话在独立受限 bridge 生成整份候选并确认，来源 transcript 不变", async t => {
  const root = await tempFixture(t, "panel-memory-api-"), data = join(root, "data"), workspace = join(root, "workspace"); await mkdir(data); await mkdir(workspace);
...
test("再次整理只发送 checkpoint 后原文，并显式合并上一版已确认会话记忆", async t => {
  const root = await tempFixture(t, "panel-memory-rolling-"), data = join(root, "data"), workspace = join(root, "workspace"); await mkdir(data); await mkdir(workspace);
...
test("pending 状态不暴露 checkpoint，scratch 或未配置 runtime 不阻止直接 compact", async t => {
  const root=await tempFixture(t,"panel-memory-status-");await mkdir(join(root,"data"));await mkdir(join(root,"workspace"));
...
test("scratch 来源与含副作用工具的 runtime 在调用模型前被拒绝", async t => {
  const root = await tempFixture(t, "panel-memory-deny-"); await mkdir(join(root, "data")); await mkdir(join(root, "workspace")); let bridgeCalls = 0;
...
test("候选生成期间离开原分支时拒绝保存候选", async t => {
  const root = await tempFixture(t, "panel-memory-branch-"); await mkdir(join(root, "data")); await mkdir(join(root, "workspace")); let branched = false;
...
test("索引刷新失败不回滚已确认记忆，重试确认会再次刷新", async t => {
  const root = await tempFixture(t, "panel-memory-index-retry-"), data = join(root, "data"), workspace = join(root, "workspace");
...
test("删除滚动文件后可选择恢复并增量整理，或从权威完整会话重建", async t => {
  const root = await tempFixture(t, "panel-memory-api-recovery-"), data = join(root, "data"), workspace = join(root, "workspace"); await mkdir(data); await mkdir(workspace);
