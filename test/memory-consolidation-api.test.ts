...
test("eligible 会话在独立受限 bridge 生成整份候选并确认，来源 transcript 不变", async () => {
  ...
  const bridge = { async generate(request: unknown) { calls.push(request); return { runId: "run", sessionId: "temporary", entries: [{ type: "message", id: "answer", parentId: null, message: { role: "assistant", content: "# Memory\n\n- Stable preference" } }] }; } } as unknown as BridgeService;
  const tools = { async configuredTools() { return { agentId: "panel-memory-agent", scope: "configured-runtime-catalog" as const, groups: [{ id: "memory", label: "Memory", source: "core" as const, tools: [{ id: "memory_search", label: "Search", description: "", source: "core" as const, defaultProfiles: [] }] }] }; } };
...
test("scratch 来源与含副作用工具的 runtime 在调用模型前被拒绝", async () => {
  ...
  const api = new PanelMemoryConsolidationApi(new MemoryConsolidationStore(join(root, "data")), { async memorySource() { return source(disposition); } },
    new Map([["agent", { runtimeAgentId: "panel-memory-agent", workspaceRoot: join(root, "workspace") }]]), { async generate() { bridgeCalls++; throw new Error("unexpected"); } } as unknown as BridgeService,
    { async configuredTools() { return { agentId: "panel-memory-agent", scope: "configured-runtime-catalog" as const, groups: [{ id: "system", label: "System", source: "core" as const, tools: [{ id: "exec", label: "Exec", description: "", source: "core" as const, defaultProfiles: [] }] }] }; } });
...
test("候选生成期间离开原分支时拒绝保存候选", async () => {
  ...
  const api = new PanelMemoryConsolidationApi(new MemoryConsolidationStore(join(root, "data")), sources,
    ...
    { async generate() { branched = true; return { runId: "run", sessionId: "temporary", entries: [{ type: "message", id: "answer", parentId: null, message: { role: "assistant", content: "candidate" } }] }; } } as unknown as BridgeService,
    { async configuredTools() { return { agentId: "panel-memory-agent", scope: "configured-runtime-catalog" as const, groups: [] }; } });
