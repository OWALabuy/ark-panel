...
test("settings HTTP 要求登录和 CSRF，并拒绝未知字段", async t => {
  ...
  const localized = await fetch(`${x.base}/api/v1/settings`, { method: "PATCH", headers, body: JSON.stringify({ locale: "en" }) }); assert.equal(localized.status, 200); assert.equal((await localized.json()).data.locale, "en");
