...
import { mkdir, symlink, writeFile, link } from "node:fs/promises";
...
test("记忆文件只列出固定 Markdown allowlist 并可安全读取", async t => {
  const root = await tempFixture(t, "panel-memory-"); await mkdir(join(root, "memory", "topics"), { recursive: true });
...
test("记忆读取拒绝 symlink、hardlink 与超限文件", async t => {
  const root = await tempFixture(t, "panel-memory-secure-"), outside = join(await tempFixture(t, "panel-memory-outside-"), "secret.md");
