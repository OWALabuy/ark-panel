...
test("只读扫描 active/reset/panel，容忍 active 半行并拒绝符号链接来源", async () => {
  ...
  assert.equal(activeRecord.memoryDisposition, "scratch");
  await reads.updateSession(activeRecord.recordId, { memoryDisposition: "eligible" });
  assert.equal((await reads.sessions("fixture")).find(item => item.recordId === activeRecord.recordId)?.memoryDisposition, "eligible");
  ...
  await reads.updateSession(activeRecord.recordId, { title: "继承后的标题", project: "Fork Project" });
  const forked = await reads.fork(listed.find(item => item.sourceKind === "active")!.recordId, "a1") as { recordId: string };
  const afterFork = await reads.sessions("fixture"); const forkRecord = afterFork.find(item => item.recordId === forked.recordId);
  assert.equal(forkRecord?.sourceKind, "panel"); assert.equal(forkRecord?.title, "继承后的标题"); assert.equal(forkRecord?.project, "Fork Project");
  ...
  const editedConversation = await reads.conversation(edited.recordId) as { title: string; project?: string; document: { entries: Array<{ message?: { content?: unknown } }> } };
  assert.equal(editedConversation.title, "继承后的标题"); assert.equal(editedConversation.project, "Fork Project");
  assert.equal(editedConversation.document.entries.length, 0, "编辑后的消息应由正常 generation run 提交");
...
test("面板会话要求显式确认并先归档才可永久删除", async () => {
  ...
  assert.equal((await reads.conversation(created.recordId) as { memoryDisposition: string }).memoryDisposition, "eligible");
