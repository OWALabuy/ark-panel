...
test("只读 sidecar 默认安全，并发更新不丢字段且不使用来源名作为文件名", async t => {
  const root = await tempFixture(t, "readonly-meta-");
  ...
  await Promise.all([
    updateReadonlyMetadata(root, identity, value => ({ ...value, title: "新标题", project: "Panel" })),
    updateReadonlyMetadata(root, identity, value => ({ ...value, archived: true, pinned: true }))
  ]);
  const loaded = await loadReadonlyMetadata(root, identity); assert.equal(loaded.title, "新标题"); assert.equal(loaded.archived, true); assert.equal(loaded.pinned, true); assert.equal(loaded.project, "Panel");
...
test("只读 sidecar 拒绝通过符号链接目录写出 dataRoot", async t => {
  const root = await tempFixture(t, "readonly-meta-link-"), outside = join(root, "outside"), data = join(root, "data");
