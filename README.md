...
Generation runs are server-owned resources rather than properties of one browser request. The panel persists their lifecycle and idempotency state, lets browsers query or re-subscribe after a dropped SSE connection, and only clears a draft after a confirmed completed run. While OpenClaw is running, the panel also relays its coalesced assistant-text updates and tool start/completion events as an ephemeral live preview. Text and tool cards are projected into their upstream sequence instead of being grouped by type; completion updates the original tool card without moving it. The pinned OpenClaw `2026.6.11` terminal tool `result` is shown in that card as plain text/JSON, with values larger than 64 KiB replaced by an omission marker. This is upstream event streaming, not a promise of one event per token; partial tool updates and reasoning are not exposed.
...
| Generation | Ordered live assistant text, tool status, and terminal result | ✅ | Interleaves coalesced text and tool cards by upstream sequence; duplicate/replayed events are idempotent; terminal results are bounded to 64 KiB while partial updates and reasoning are not streamed |
