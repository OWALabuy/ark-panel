...
By default the panel reads the local URL and token/password from `~/.openclaw/openclaw.json`. For that same resolved endpoint, local Gateway config follows the pinned resolver: explicit `mode=none` always disables this connection even if stale token/password fields or panel environment credentials remain; `token` and `password` use only their matching field; `trusted-proxy` uses only its same-host password fallback; and an omitted mode is inferred only when exactly one configured credential kind exists (both is ambiguous and rejected). A declared `PANEL_OPENCLAW_GATEWAY_TOKEN` / `PASSWORD` group overrides only the credential selected by that known mode; an all-blank group fails closed without falling back to the file. Remote-mode credentials remain a separate `gateway.remote` input and do not inherit local `gateway.auth.mode`. Only when `PANEL_OPENCLAW_GATEWAY_URL` resolves to a different WebSocket origin may a non-blank panel credential group act as an independent endpoint assertion; the URL alone never forwards a configured secret. This assertion does not change the target Gateway's server-side auth mode, so deploy it only together with a Gateway that enforces the matching token/password and validate it under #48. Treat the selected shared secret as an owner-level credential for one trusted operator, not as multi-tenant isolation, and keep the default loopback route. Even though upstream can allow unauthenticated direct-loopback access, the panel refuses an auth-none admin connection. An explicitly configured same-owner private URL remains parse-compatible, but it does not inherit direct-loopback self-pairing and is outside this local-connection guarantee until separately paired and accepted upstream. Rotate the secret in the Gateway and panel together, restart both sides, and roll both back together if needed. This release does not require a credential reissue. If the server cannot resolve a usable Gateway credential, read-only panel access remains available while every Gateway control operation fails with the stable `GATEWAY_TRANSPORT_UNAVAILABLE` code; production never falls back to a per-request Gateway CLI connection. `PANEL_OPENCLAW_STREAMING=0` disables only the ephemeral text/tool preview: the same control WebSocket and all three scopes remain necessary for generation, typed commands, attachments, and temporary-session lifecycle. A preview failure cannot decide run completion, but an unavailable control connection fails operations that need a Gateway RPC. The completed, verified transcript remains authoritative and replaces any preview atomically.
