...
| 上下文 | 持久压缩与 `/compact` | 🚧 | 面板自建会话后端已能持久采纳经验证的 OpenClaw 压缩记录且不删除历史；UI 标记与有效预算投影仍在排期 |
...
| 记忆 | eligible 会话的增量整理 | ✅ | 每会话一份滚动记忆；确认后原子替换，并刷新真实 Agent、聊天 runtime 与记忆 runtime 的 OpenClaw 索引 |
...
外观、侧栏、头像、标题、对话状态、后台完成、双语界面和可审阅记忆流程已经完成；之后再处理带 `/compact` 的持久长上下文策略。`scratch` 仍完整读取既有记忆，只是不进入面板沉淀流程；详细边界见[记忆模块决定](docs/decisions/panel-memory.md)。OpenClaw 兼容性属于持续的日常维护。体验功能的取舍理由见[体验功能决策记录](docs/decisions/ux-features.md);详细的约束和验收标准见[实现规格说明](docs/implementation-spec.md)。
