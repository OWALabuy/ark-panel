...
| 记忆 | eligible 会话的增量整理 | 🚧 | 手动提炼未整理消息、预览确认后写独立短期记忆文件；OpenClaw 继续负责索引与 promote |
...
外观、侧栏、头像、标题、对话状态、后台完成和双语界面这几批工作已经完成。近期顺序是记忆处置 UI、只读记忆中心和 eligible 会话的可审阅增量整理，随后再处理带 `/compact` 的持久长上下文策略。`scratch` 仍完整读取既有记忆，只是不进入面板沉淀流程；详细边界见[记忆模块决定](docs/decisions/panel-memory.md)。OpenClaw 兼容性属于持续的日常维护。体验功能的取舍理由见[体验功能决策记录](docs/decisions/ux-features.md);详细的约束和验收标准见[实现规格说明](docs/implementation-spec.md)。
