...
默认情况下，面板从 `~/.openclaw/openclaw.json` 读取本机 URL 和 token/password。对同一个已解析端点，本机 Gateway 配置遵循固定上游版本的 resolver：显式 `mode=none` 即使残留 token/password 字段或面板环境凭据也始终禁用该连接；`token` 和 `password` 模式只采用对应字段；`trusted-proxy` 只采用同机直连所需的 password fallback；未写 mode 时仅在配置中恰有一种凭据时自动推导，同时存在两种凭据属于歧义并拒绝。声明 `PANEL_OPENCLAW_GATEWAY_TOKEN` / `PASSWORD` 只覆盖该已知 mode 选中的凭据；若声明的整组值全为空白，则 fail closed，绝不回落到配置文件。remote mode 的凭据仍是独立的 `gateway.remote` 输入，不继承本机 `gateway.auth.mode`。只有当 `PANEL_OPENCLAW_GATEWAY_URL` 解析为不同的 WebSocket origin 时，非空面板凭据组才能作为独立端点断言；只有 URL 而没有独立凭据时绝不会转发配置中的 secret。这种断言本身不会改变目标 Gateway 服务端的 auth mode，因此只能与确实强制对应 token/password 的 Gateway 同步部署，并留待 #48 验证。这份最终选中的共享密钥是单一受信 operator 的 owner 级凭据，不是多租户隔离边界，并应保留默认 loopback。即使上游允许 direct-loopback 无认证访问，面板也拒绝建立 auth-none 的 admin 连接。显式配置的同一 owner 可信私网 URL 仍保持解析兼容，但不会继承 direct-loopback self-pairing；在另行完成上游配对与验收前，它不属于本机连接保证。轮换时必须同步更新 Gateway 与面板并重启两端，回滚时也同步恢复两端；本次发布不要求重新签发凭据。若服务端无法解析出可用的 Gateway 凭据，面板只读访问仍可用，所有 Gateway 控制操作以稳定的 `GATEWAY_TRANSPORT_UNAVAILABLE` 失败；生产不会回落为逐请求 Gateway CLI 连接。`PANEL_OPENCLAW_STREAMING=0` 只关闭临时文本/工具预览，同一控制 WebSocket 和三个 scope 仍用于生成、typed 命令、附件和临时 session 生命周期。预览失败不能决定 run 是否完成，但控制连接不可用时，需要 Gateway RPC 的操作会失败。经过校验的完整 transcript 始终是权威版本，并以原子方式替换任何预览。
