...
- 服务端用同一个进程内读取索引覆盖 active/reset/panel：目录清单和安全 stat 指纹只决定缓存失效，document 与 metadata 仍从权威文件验证。列表与搜索各取一次快照，新增/变更记录至多解析一次；recordId 已定位后只探测目标。附件 owner 查询复用同一 locator。进程重启、缓存清空或坏缓存都从权威来源重建，不产生持久索引格式。
