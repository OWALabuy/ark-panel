...
- 首版提供列表、查看、安全 Markdown 渲染，以及面板生成记忆的来源会话跳转；不提供任意 workspace 浏览和在线编辑。
...
- 候选提炼使用独立的内部 job/run 和一次性 session，沿用来源会话的有效 modelOverride、thinkingLevel 与 reasoningLevel。不得复用普通聊天 runId，不得把内部 user/assistant/tool/reasoning entries 提交到来源 transcript。
...
- 提炼结果先作为带 batch id、来源 revision、throughEntryId 和内容 hash 的候选保存到 `PANEL_DATA_DIR`，不触碰 workspace、不推进 checkpoint。
- 候选按整份 Markdown 预览和确认；首版不拆逐条候选卡片，也不提供逐条勾选/编辑。
- 确认接口复核：来源仍为 `eligible`、候选 revision/内容 hash 未变、batch 未确认。成功后以唯一且不覆盖的 `memory/YYYY-MM-DD-ark-panel-<batch>.md` 写入目标 workspace；不得与 OpenClaw 共写日期主文件，不得直接修改 `MEMORY.md` / `DREAMS.md`。
...
- OpenClaw watcher/index/promote 接手已确认的短期文件；面板不复刻 promote。删除短期源文件不能承诺撤销已被 promote 到 `MEMORY.md` 的副本。
...
- 确认成功只新增一个符合命名规则的短期文件，ledger/checkpoint 与文件一致；重复确认不产生第二份，任意故障点不允许 checkpoint 超前。
...
19. **纯本地增量（不连 gateway，可随时做，与排序无关）：**
    ...
    - §5.5 记忆倾向标记的存储已完成；仍需补 UI（打标本身不连 gateway）。
20. **记忆模块（分批实施）：**
    - 20a（已完成）：处置状态 UI；明确 `scratch` / `eligible` 都读取既有记忆，仅后者可进入沉淀流程。
    - 20b（已完成）：只读记忆中心，安全列出和查看目标 agent 的 `MEMORY.md`、`DREAMS.md` 与 `memory/**/*.md`。
...
- MemoryCenter（后续：只读记忆文件列表、内容查看、面板贡献的来源跳转）
- MemoryCandidateReview（后续：eligible 会话的固定范围候选预览与确认；未确认不写 workspace）
