...
23. **附件/多模态输入与模型产出文件（已实现）：** 文件选择、图片粘贴和拖放均先上传到面板私有存储；run 只提交附件 ID，服务端复核会话归属后把原始字节交给 OpenClaw。Office 不转换，模型按需用 Python/skill 读取。模型产出只采集本轮 artifact 与可信 workspace 下的隔离 outputs 目录，复制入面板存储后清理暂存。草稿仍仅在浏览器本地。
