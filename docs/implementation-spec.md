...
22. **长上下文完整策略（含 B 类 `/compact`，已完成）：** 面板自建会话物化临时 session 后调用 `sessions.compact` typed RPC，读回并严格验证新增 compaction 后采纳进面板存储，绝不把 `/compact` 送进 `sessions.send`。首版采用手动时机；完整旧消息继续可见，最新 compaction 决定有效上下文；fork 在边界前不继承摘要、在边界及之后继承摘要。eligible 的记忆提醒复用 §5.5 的显式候选/确认/ledger 路径，scratch 不沉淀。
