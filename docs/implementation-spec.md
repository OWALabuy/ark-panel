...
- **原子更新与并发**：sidecar 写用 `atomicWrite`（临时文件 + rename）；面板服务是单写者，无跨进程竞争（gateway 不知道这个目录）。
...
| OpenClaw 自带命令 | 普通消息路径永久拒绝 `/`；命令走独立派发接口，首版做 A（面板原生）+ C（只读信息类），D 类默认不做（`/bash` 部署可选）。见 §5.4 与 `decisions/slash-commands.md` | C 类只读 RPC 相关 |
...
| C 信息类（`/help` `/commands` `/status` `/models` `/tools` `/usage`） | ✅ | 调只读 RPC，面板渲染，不产生写入。**但它们不是同质数据，逐命令来源/差异见下表。** |
| D gateway 管理 / owner 全局（`/config` `/restart` `/reset` `/mcp` `/plugins`） | ❌ 默认不做 | 面板不暴露入口；用户在原生 surface 执行。 |
| `/bash`（**面板原生能力**，非命令代理） | 部署可选，默认关；安全设计未落定前不进实现批次 | gateway 无命令执行 RPC，`/bash` 不可能映射到 typed RPC，唯一自洽做法是**面板自己执行 shell**。故 `/bash` 是一项面板原生能力，`/bash` 只是其 type-to-invoke 触发名。走独立派发路径，绝不经普通消息路径、绝不把命令字符串送进 `sessions.send`。开启需登录已启用。完整安全设计见 **§5.6**。 |
...
- 面板实际支持的命令，是一份**显式、版本化、默认拒绝（default-deny）的服务端 allowlist**（首版 = A 类 + C 类枚举 + 部署开启时的 `/bash`）。补全列表里出现但不在 allowlist 的命令（如 `/skill`、`/goal`、`/tasks`、`/approve`、`/export-session` 等），用户选中后服务端以稳定错误码拒绝，不派发。
...
|---|---|---|---|---|---|
| `/model [m]` | 返回当前 override + 可选模型列表（源同 `/models`），不报错 | 写 `modelOverride` | 值须在 `/models` 目录内，否则拒绝；模型标识用 gateway 目录里的规范形式（`provider/model` 或其别名），面板不自造 | `/model default` 或空值清除 → 删除 `modelOverride`，回 agent 默认 | 写 metadata 即时生效，作用于**下一轮**推理（不影响进行中的 run） |
| `/think [level]` | 返回当前 `thinkingLevel` + 可选值 | 写 `thinkingLevel` | 值须在 gateway 支持集内；当前模型不支持该 level 时**派发即拒绝并回报原因**，不静默降级 | 空值清除 → 回默认 | 同上，下一轮生效 |
| `/reasoning [level]` | 同 `/think` 语义（reasoning-effort 维度） | 同上 | 同上 | 同上 | 同上 |
...
| `/models` | 模型目录 + provider 信息（`status` 或专用只读 RPC，实现时核实来源） | 若 `status` 不含完整模型目录，需另找只读来源；来源在实现时按 2026.6.11 源码核实并记注释 |
| `/tools` | 当前 agent/runtime 的工具上下文（**`commands.list` 不提供**，需另取或由面板按 2a′ 装配清单生成） | 面板展示的是**本次推理会装配的工具集**，与 gateway 内部视图可能不完全一致 |
| `/usage` | 当前会话/token 使用（来源实现时核实；可能需按面板 transcript 估算） | 若 gateway 不提供 per-panel-session usage，面板按自有 transcript 估算并标注为估算值 |

> C 类落地前提：每个命令的**具体 RPC、参数、返回 DTO** 在实现时逐一核实（2026.6.11 源码），来源不明确的（`/models`/`/tools`/`/usage`）以核实结果为准，并把「面板值 vs 原生值」的差异写进代码注释与本表。
...
- C 类：`/commands`、`/status` 返回 gateway 的真实数据并正确渲染；这些调用只用 `operator.read`，不产生任何写入或 transcript 变更。
- **allowlist 默认拒绝**：补全列表里出现但不在 allowlist 的命令（如 `/skill`、`/goal`、`/tasks`、`/export-session`），选中后服务端以稳定错误码拒绝、不派发；构造升级后新增的未知命令名派发，同样默认拒绝。
- D 类命令（除 `/bash`）在面板无入口；即便手工构造派发请求，服务端也拒绝。
- `/bash` 默认关闭时派发被拒绝；仅在部署显式开启后可用，且开启需要登录已启用（详见 §5.6）。
...
### 5.6 `/bash`：面板原生 shell 执行（部署可选，安全设计未落定前不实现）

`/bash` 是**面板原生能力**而非 gateway 命令代理（原因见 [`decisions/slash-commands.md`](decisions/slash-commands.md)：gateway 无命令执行 RPC，唯一自洽做法是面板自执行 shell）。作为部署可选功能开放，但**必须先落定下列安全设计，否则不进入实现批次**。首版可以先不做 `/bash`，只保留这份规格与「默认关闭 + 派发被拒绝」的行为。

启用开关：
- 默认**关闭**；由部署配置显式开启（如 `PANEL_ENABLE_BASH=1`）。
- **启动期校验**：开启 `/bash` 但登录未启用（或登录强度未达安全基线）时，服务**拒绝启动并报错**，不允许「开了 bash 又没登录」的组合存在。

执行模型（实现时逐条定死并测试）：
- **shell 与 cwd**：用哪个 shell、默认工作目录（建议限定在明确的、非 gateway 敏感目录），是否允许 per-命令指定 cwd 及其边界校验（沿用 §0 路径/符号链接防护，拒绝越界）。
...
- **参数编码 / 防注入**：结构化 `{ command, args }` 派发，参数以数组形式交给执行器（不拼接进 shell），避免命令注入。
...
12. **斜杠命令（见 §5.4）**：普通消息路径永久拒绝 `/`（保留客户端与服务端双重拒绝测试）；命令走独立派发接口，首版做 A（面板原生）+ C（只读信息类），按 default-deny allowlist 校验。`/compact` 归 §22 长上下文（经 `sessions.compact` typed RPC），`/reset` 等 D 类默认不做，`/bash` 待 §5.6 安全设计落定。
...
19. **纯本地增量（不连 gateway，可随时做，与排序无关）：**
    - §8.7 Markdown 渲染 + 消息复制 + 代码块复制。
    - 消息时间戳 UI 显示（读 entry `timestamp`，本地时区）。
    - §4.3 会话重命名 / 归档 / 取消归档 / 删除（面板会话彻底删除、只读会话仅隐藏）。
    - §5.5 记忆倾向标记的**存储与 UI**（打标本身不连 gateway）。
20. **记忆控制层的推理侧（连 gateway，`paneltest` 验）：** §5.5 `scratch` 会话的记忆副作用收窄，二选一实测确认。
21. **斜杠命令适配（首版做 A + C 两类）：** 按 [`decisions/slash-commands.md`](decisions/slash-commands.md) 的分类适配设计与 §5.4 实现规格。守住两条路径隔离的安全不变量（普通消息路径拒绝 `/`，命令只走独立结构化派发接口，命令字符串绝不带内进 `sessions.send`）。
    - A 面板原生（`/model` `/think` `/new`）：写面板会话 metadata，推理时应用；与 §8.6 换模型同一实现。
    - C 信息类（`/help` `/status` 等）：只读 RPC 渲染。
    - `/` 补全拉 `commands.list`（含 skill/plugin 动态命令）。
    - B `/compact` 归第 22 项；D 类默认不做，`/bash` 作为部署可选（默认关）。
22. **长上下文完整策略（含 B 类 `/compact`）：** 在第 21 项之后。活会话压缩交给 gateway；**面板自建会话的 `/compact` 与长上下文压缩是同一份工作**——做法是跑一次压缩推理、读回压缩后 transcript、采纳进面板存储。需定：压缩时机、摘要 entry、fork 穿越摘要边界、压缩点是否触发一次侧重 `memory_search` 的推理以补记忆归档。这也是记忆路线 B「按需喂 promote」可能落地的点。
