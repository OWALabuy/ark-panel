...
- `archived`：布尔。归档=移入归档列表，取消归档=移回主列表。归档状态在重新扫描源目录后仍生效，被归档/隐藏的只读会话不因 rescan 重新出现在主列表。
- `memoryDisposition`：`eligible`｜`scratch`（默认值与语义见 §5.5 与 `decisions/panel-memory.md`）。
- 只读会话（`sourceKind` = `active` / `reset`）的“删除”在数据层等同于 `archived`/隐藏；**没有任何路径可 unlink、改名或改写其源文件**。
- 面板自建会话（`sourceKind` = `panel`）的彻底删除：必须先 `archived`，再经显式二次确认，才移除面板拥有的 transcript 与 metadata；git（git-crypt）历史为最后兜底。

验收：
- 重命名活会话与 reset 归档：面板记录的 `title` 改变，源文件字节逐字未变（改动前后对源文件做 diff 应为空）。
- 归档某会话后，它从主列表消失、出现在归档列表；取消归档后回到主列表。删除 `index.json` 重建后，归档状态保持一致（说明状态不只存索引）。
- 只读会话执行“删除”：源文件仍在原处、内容未变；面板主列表与归档列表都不再默认展示它（除非在归档列表查看）。
- 面板自建会话未归档时，彻底删除入口不可用（或被拒绝）；归档并二次确认后，面板 transcript 与 metadata 被移除，且该操作不触碰任何非 `panel` 会话。
- 路径与符号链接防护沿用 §0 与工程决定：删除只作用于由 agentId + recordId 推导出的面板数据目录内文件，拒绝路径越界与符号链接。
...
| 会话归档 / 取消归档 | 面板 metadata 的 `archived` 状态；主列表与归档列表分开；rescan 不复现被归档的只读会话。见 §4.3 | 否 |
| 会话删除 | 只读会话=隐藏（等同归档，永不碰源文件）；面板自建会话=归档后经二次确认彻底删除面板文件，git 兜底。见 §4.3 | 否 |
...
| 类 | 首版 | 实现 |
|---|---|---|
| A 面板原生（`/model` `/think` `/reasoning` `/new`） | ✅ | 写入面板会话 metadata（`modelOverride` / `thinkingLevel` 等）；下轮推理经 `sessions.create` 参数或 `sessions.patch` 应用到临时 session。`/new` 复用新建面板会话。与 §8.6「会话中途换模型」同一实现。 |
| B `/compact` | 归 §22 长上下文 | 跑一次压缩推理、读回压缩后 transcript、采纳进面板存储。不在本节单独实现。 |
| C 信息类（`/help` `/commands` `/status` `/models` `/tools` `/usage`） | ✅ | 调只读 RPC（`commands.list` scope `operator.read`、`status` 等），面板渲染返回结果，不产生写入。 |
| D gateway 管理 / owner 全局（`/config` `/restart` `/reset` `/mcp` `/plugins`） | ❌ 默认不做 | 面板不暴露入口；用户在原生 surface 执行。 |
| `/bash`（D 类例外） | 部署可选，默认关 | 由部署配置开启（如 `PANEL_ENABLE_BASH=1`）；仍走独立派发路径，绝不经普通消息路径。开启时登录强度是前提（见安全基线）。 |

**命令补全**：客户端在输入框敲 `/` 时拉 `commands.list`（含 skill/plugin 动态命令）+ 面板原生命令做补全。`commands.list` 不返回 owner/surface/run 可用性，面板按四分类决定暴露哪些、以及派发时的处理。
...
- `/bash` 默认关闭时派发被拒绝；仅在部署显式开启后可用，且开启需要登录已启用。
