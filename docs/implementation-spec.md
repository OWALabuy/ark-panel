...
**状态流与 transcript 提交分离**：run 生命周期是面板持久化的服务端状态，SSE 只是可丢失、可重连的观察通道；**面板自己的 transcript 文件在这次 run 结束前一个字都不写。** run 正常结束、拿到完整的一组 entry 后，先将待提交结果落到受限 run 记录，再以临时文件 + fsync + rename 原子替换提交完整 transcript。gateway 后续若提供逐 token 事件，可再扩展 SSE，但不改变提交语义。
