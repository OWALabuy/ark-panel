...
14. **持久 `/compact` 后端（已实现，交互与预算闭环待完成）**：仅允许面板自建会话。服务端以拒绝排队的同会话独占操作锁定 revision，物化 OpenClaw 2026.6.11 可识别的当前分支，在一次性 runtime 应用当前模型、thinking 和 reasoning override 后调用 `sessions.compact` typed RPC。普通消息路径仍拒绝所有 `/` 文本。面板不信任 RPC 摘要 DTO：它重新读取原临时 transcript，要求历史 entry 前缀逐项完全一致、只新增一个结构和父链合法的 `compaction`、`firstKeptEntryId` 位于该分支或指向 compaction 自身，随后才以现有 durability helper 原子追加到完整面板 transcript。原始消息永久保留。OpenClaw 即使返回 rotation successor，也只从原 transcript 的已验证追加采纳；successor id 和文件必须同时匹配受信 runtime 根，官方注销后才严格清理两组 artifacts。原文件缺失、改写、异步 pending、revision race 或无效边界均稳定失败且不修改面板权威数据。transport 超时或断线属于 outcome unknown：面板先请求 abort/release，只有确认释放后才清理；无法确认时保留临时 session 并返回稳定错误，避免与仍运行的压缩竞争。当前尚无 compact orphan 的持久 journal，因此这种极少数保留项需要按 runtime artifact 运维流程检查；不得自动删除。当前工作项不改变上下文预算算法，也不提供自动压缩。
...
22. **长上下文完整策略（含 B 类 `/compact`）：** 在第 21 项之后。活会话压缩交给 gateway；**面板自建会话的 `/compact` 与长上下文压缩是同一份工作**——物化临时 session 后调用 `sessions.compact` typed RPC，读回压缩后的 transcript 并采纳进面板存储，绝不把 `/compact` 送进 `sessions.send`。需定：压缩时机、摘要 entry、fork 穿越摘要边界，以及 eligible 会话是否在压缩前提示运行 §5.5 的显式记忆整理。压缩本身不得暗中沉淀 scratch 会话，也不另建一条绕过候选确认与 ledger 的记忆写路径。
23. **附件/多模态输入与模型产出文件（已实现）：** 文件选择、图片粘贴和拖放均先上传到面板私有存储；run 只提交附件 ID，服务端复核会话归属后把原始字节交给 OpenClaw。Office 不转换，模型按需用 Python/skill 读取。模型产出只采集本轮 artifact 与可信 workspace 下的隔离 outputs 目录，复制入面板存储后清理暂存。图片草稿使用浏览器本地 Blob URL 缩略图；已提交图片只通过登录鉴权后的专用接口内联真实解码、有界的单帧 PNG/JPEG/WebP，点击可在灯箱查看并下载原图。
...
- 草稿按 agent + session 存在浏览器本地，刷新或切回会话时恢复；发送及结构化命令成功后清除，失败时保留。localStorage 不可用或超额时静默降级，不影响发送。
