...
- 共享 secret 是单一 trusted-local operator 的 owner 级凭据，不是多租户隔离。同一本机端点的凭据选择必须与固定 OpenClaw resolver 对齐：`gateway.auth.mode=none` 无条件禁用 admin 连接；`token` / `password` mode 只取对应字段；`trusted-proxy` 只允许同机 direct fallback password，配置 token（明文或 SecretRef presence）一律拒绝；mode 未设置时，只有配置 token 或只有配置 password 才推导，同时存在两者因歧义拒绝。已声明的面板环境凭据组只覆盖该 mode 选中的值，整组全为空白不能回落。
- 本机端点从 `gateway.port` 与 `gateway.tls.enabled` 推导；origin 以 scheme、规范化 port 和 discriminated host identity 比较。localhost、127/8、`::1` 与 IPv4-mapped loopback 是同类 alias，真实 DNS hostname（包括字面 `loopback`）属于另一类，禁止用合法 hostname 充当 sentinel。公网 hostname/IP 的 `ws://` 必须拒绝；明文只允许 loopback、RFC1918、link-local、CGNAT、IPv6 ULA、`.local` 与 `.ts.net`，其余必须 `wss://`。TLS 证书验证不能关闭。
- remote mode 必须有非空且安全的 URL、显式 `transport: "direct"`，不能回落本机默认地址；面板不建立 SSH tunnel、不实现 remote `tlsFingerprint` pin，遇到二者拒绝。`gateway.remote` credential 独立于本机 auth mode。任何显式 panel URL 都必须带非空 panel credential group，不得继承 remote/local 配置 secret 或 pin；本机模式仅在显式 URL 与配置 origin 不同时采用同一独立边界。该配置只是操作员断言，不改变目标服务端 mode，须同步确保目标端强制对应 secret，并由 #48 验收。
...
- 本机 `gateway.auth.mode=none` 带任意遗留或同端点 `PANEL_*` secret 时都不创建 admin socket；token/password mode 只发送对应凭据，trusted-proxy+token 或缺 password 拒绝。SecretRef presence 参与单凭据推导、双凭据歧义和互斥；选中 ref 无对应 panel override 时拒绝。
- TLS 本机配置生成 `wss://`；loopback aliases 同源，真实 hostname `loopback` 不继承本机 secret。loopback、private、link-local、CGNAT、ULA、`.local`、`.ts.net` 明文正例与公网 hostname/IPv4/IPv6 明文负例均有覆盖。
- remote URL 缺失、空串或空白时，无论 remote/PANEL token、password 或两者都不回落本机且不创建 socket；SSH/默认 transport、非法 transport 与已配置 fingerprint 同样拒绝。显式 URL 只有和非空 panel credential group 一起才可作为独立端点。已配置 direct remote URL 的 token/password/both 保持独立解析，URL-only override 不带走配置 secret。所有拒绝保持稳定 code 且不泄露凭据、ref、配置正文或路径。
