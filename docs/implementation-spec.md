...
**工具写入边界：** 当前协议不能在保留 shell/文件工具写项目能力的同时，按普通 session 或路径禁止模型直接写 memory 文件。首版至少保证面板管理的沉淀管线不接受 scratch；是否要求隔离/COW workspace 来阻止模型工具直接写入，须由 `decisions/panel-memory.md` 的开放决策落定后才可宣称为硬保证。
...
20. **记忆模块（分批实施）：**
    ...
    - 20c：eligible 会话的增量候选提炼、预览确认、唯一短期文件写入、ledger/checkpoint 与失败恢复。
