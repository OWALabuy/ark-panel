...
21. **斜杠命令适配：** 按 `decisions/slash-commands.md` 六条原则；命令目录与执行来自 gateway 官方分派，面板不复刻。先落地 gateway 已能安全代理的命令，`/compact` 等改变会话生命周期的命令要先想清对面板索引与 transcript 的影响。
22. **长上下文完整策略：** 在第 21 项之后。活会话压缩交给 gateway；面板自建会话的压缩策略（时机、摘要 entry、fork 穿越摘要边界、压缩点是否触发一次侧重 `memory_search` 的推理以补记忆归档）由面板自己定。这也是记忆路线 B「按需喂 promote」可能落地的点。
