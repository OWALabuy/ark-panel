...
1. **普通聊天的记忆配置不随处置状态变化。** `scratch` 与 `eligible` 必须配置为使用同一目标 workspace、bootstrap 与工具策略；面板不得因为会话不沉淀而主动删减记忆能力。共享配置不能证明具体 runtime 实际注入或召回记忆，当前逐 runtime 状态在 #48 前为 unknown。
2. **`memoryDisposition` 只控制当前会话能否进入面板管理的记忆沉淀流程。** `scratch` 不会被面板提炼、写入或定时整理；只有 `eligible` 才能产生候选记忆。
3. **面板为每条会话实现一份可审阅、滚动修订的短期记忆事务。** 首次整理以完整当前分支为候选输入；后续使用「上一版已确认记忆 + checkpoint 后新增原文」。用户确认后，面板原子创建或替换该会话唯一的 `memory/ark-panel/<record-key>.md`；不直接编辑 `MEMORY.md` 或 `DREAMS.md`。事务边界有确定性覆盖，真实候选模型执行仍待 #48。
4. **记忆中心保持只读，自动整理不在当前范围。** 用户可以查看 allowlist 内的记忆文件和面板贡献的来源；在线编辑、定时自动确认与长期记忆回滚不受支持。
5. **删除滚动文件不等于要求遗忘。** 用户可能已把内容整合进长期记忆，只是为了保持 workspace 整洁而移除面板短期文件。下次手动整理时由用户选择：以最后确认的完整面板候选恢复并继续增量整理、忽略旧面板记忆并从权威完整会话重新生成，或暂不处理；面板不静默复活文件，也不静默清空 checkpoint。

这次修订推翻旧版「`scratch` 不装配记忆工具或改用无记忆 workspace」的决定。该做法
违反两种处置状态采用相同连续性配置的产品合同，而且固定 OpenClaw 版本的普通 session
也不支持按单次请求设置工具 deny。
...
| 概念 | 含义 | 当前行为 |
|---|---|---|
| 记忆读取 | 模型使用 `MEMORY.md`、近日记忆和记忆搜索回答当前问题 | 两种状态配置相同；实际逐 runtime 状态 unknown（#48） |
| 会话处置 | 用户是否允许面板从这条会话提炼新记忆 | 每会话 `scratch` / `eligible`，默认 `scratch` |
| 记忆沉淀 | 把尚未整理的对话提炼成短期记忆文件，交给 OpenClaw 索引和后续 promote | 仅 `eligible`；手动、预览后确认 |

`eligible` 的含义是「允许整理」，不是「每条消息自动写记忆」；`scratch` 的含义是「不进入面板的沉淀管线」，不是「禁用已有记忆」。

## OpenClaw `2026.6.11` 的固定版本假设与验收状态
...
- `panel-runtime-<agent>` 按合同与目标 agent 共用 workspace，并使用隔离的 runtime session 目录；共用 workspace 本身不足以证明正常面板推理获得相同 bootstrap、记忆文件或记忆工具。每个 runtime 必须由 #48 独立验证。
...
- 若某个 runtime 经验证实际获得 `memory_search`，调用它可能留下查询、召回或强化统计供 OpenClaw 的记忆流程使用。这不是把当前会话正文写成新记忆，但也意味着「读取记忆」不能承诺内部状态字节级零变化。
...
| 状态 | bootstrap/记忆配置合同 | 工具配置合同 | 面板手动整理 | 面板定时整理 |
|---|---:|---:|---:|---:|
| `scratch` | 与 `eligible` 相同；实际 unknown | 与 `eligible` 相同；实际 unknown | 拒绝 | 拒绝 |
| `eligible` | 与 `scratch` 相同；实际 unknown | 与 `scratch` 相同；实际 unknown | API 允许；runtime unknown | 未实现 |
...
当前实现能可靠保证：`scratch` 不会进入面板的候选提炼、确认写入或未来的定时整理队列；面板自己的记忆写 API 必须拒绝 `scratch` 来源。

产品合同不因 `scratch` 主动限制普通聊天原本有效的 agent 能力。若某 runtime 实际获得
shell / 文件工具，用户明确要求模型写记忆时允许发生；`memoryDisposition` 不拦截这种
显式工具操作。具体 effective tools 与 bootstrap 仍须 #48 逐 runtime 验证。这里的
“不整理”只约束面板自己的候选、确认和自动任务，不能被描述成 workspace 的通用只读
安全边界。

这不适用于面板发起的候选提炼内部 session：预览确认语义要求确认前 workspace 不变，
所以该内部 session 必须采用只读/无副作用工具策略，并拒绝产生任何写工具轨迹。
OpenClaw 普通 session 不能动态 deny 工具；当前实现要求独立 memory runtime，并在每次
创建内部 session 后查询 `tools.effective`，仅接受无工具或 `memory_search` /
`memory_get`。这道失败关闭门禁不能退化成“只在提示词里说不要写”，也不等于真实
runtime 已通过；每个 runtime 的当前状态仍需 #48 复验。
...
- 不在线编辑任意记忆文件，不提供任意 workspace 文件浏览器，也不把记忆正文写入服务日志。
...
3. 在**独立的一次性内部 session** 中生成候选 Markdown，沿用来源会话的有效模型、thinking 和 reasoning 设置。首次整理把固定范围历史作为输入；后续整理把该会话上一版已确认记忆明确放入输入，再附上固定范围内的新增原文，要求模型输出更新后的完整会话记忆，而不是仅输出追加片段。面板在 session 创建后读取其 `tools.effective`，仅接受无工具或 `memory_search` / `memory_get`，并拒绝 shell、文件写入或其它有副作用的工具；正确性不能依赖 OpenClaw 隐式注入旧记忆。这是失败关闭合同，真实 effective tools 与模型执行仍待 #48 验收。
...
7. 文件完成持久化后，面板按实现合同通过版本门禁内的结构化 CLI，依次请求刷新真实 agent、普通聊天 runtime 与记忆整理 runtime 各自独立的 OpenClaw 索引。不能只依赖文件 watcher：对应记忆管理器未驻留时，固定版本可能漏掉文件事件。三个命令返回成功后确认接口才报告“已建立索引”；索引失败不回滚已持久化的记忆或 checkpoint，重复确认同一 batch 会幂等地再次尝试刷新。命令成功是否对应当前部署中的真实可召回索引仍待 #48 验收；是否最终 promote 到 `MEMORY.md` 由 OpenClaw 决定，面板不复刻 promote 算法。

滚动文件缺失时，候选生成 API 只接受服务端枚举的 `incremental` / `restore` / `rebuild` 模式，浏览器不能提交路径、checkpoint、正文或基线 hash。`restore` 以 `PANEL_DATA_DIR` 中最后一个已确认、且与 ledger/current hash/target/checkpoint 全部匹配的完整候选为恢复快照；保留旧 checkpoint，只把其后的新增原文交给模型。没有新增原文时直接把该快照作为可审阅候选，确认后仍可重建文件。`rebuild` 不使用旧面板记忆，从当前权威完整分支起点生成且不得截断或跳过历史。当前候选路径不复用普通聊天的 `ConservativeContextBudget` preflight，因此不承诺 `CONTEXT_BUDGET_EXCEEDED`；实际容量与失败行为待 #48 验收。恢复快照缺失、版本不支持、损坏或文件不安全时，只允许 `rebuild`。

两种恢复都保持两阶段边界：预览只写面板候选，不修改 workspace 或 checkpoint；确认时复核来源固定范围、state 指纹、checkpoint、基线 hash，以及目标仍然缺失。若生成期间滚动文件被重新创建、修改为其它内容，或 state/来源分支变化，候选过期并拒绝覆盖。确认才原子创建文件、持久化 state/ledger，并请求刷新三份索引；写文件后进程退出但 state 未推进时，重复确认可按候选 hash 完成恢复。真实索引效果仍由 #48 判断。
...
- 检测到滚动文件缺失时，UI 提供「恢复面板记忆并继续整理」「从会话重新生成」「暂不处理」。缺失本身不使用“忘记”措辞，也不再作为普通的 `MEMORY_FILE_NOT_FOUND` 暴露给整理流程。
...
- 已确认的完整 v2 候选文件现在同时是恢复快照，备份和清理不得只保留 state/ledger 而删除这些候选。现有 v2 state 无需迁移；缺失候选只会把能力降为完整会话重建。新候选增加的恢复模式、state 指纹和预期目标状态是向后兼容字段，state 仍为 v2。回滚到不支持恢复的版本前无需改写 workspace/state；可删除尚未确认的恢复候选并重新整理，但旧版本遇到缺失滚动文件仍只会报错，因此应先在新版确认恢复/重建，或恢复备份中的滚动文件。

## 后续自动整理

自动整理若立项，必须复用同一套增量提炼、候选和 ledger，不另建一条隐式写路径：
...
3. 提炼沿用来源会话的有效模型与思考设置，但运行在独立内部 session；任何提炼、推理和工具轨迹都不进入原会话上下文。
4. 整份预览、整份确认候选 Markdown，不做逐条勾选或逐条编辑。

自动整理的执行时间与是否允许自动确认尚未决定；当前未实现。
