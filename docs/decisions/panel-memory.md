...
滚动文件缺失时，候选生成 API 只接受服务端枚举的 `incremental` / `restore` / `rebuild` 模式，浏览器不能提交路径、checkpoint、正文或基线 hash。`restore` 以 `PANEL_DATA_DIR` 中最后一个已确认、且与 ledger/current hash/target/checkpoint 全部匹配的完整候选为恢复快照；保留旧 checkpoint，只把其后的新增原文交给模型。没有新增原文时直接把该快照作为可审阅候选，确认后仍可重建文件。`rebuild` 不使用旧面板记忆，从当前权威完整分支起点生成；若完整分支超过既有上下文安全预算，诚实返回稳定预算错误，不得截断或跳过历史。恢复快照缺失、版本不支持、损坏或文件不安全时，只允许 `rebuild`。

两种恢复都保持两阶段边界：预览只写面板候选，不修改 workspace 或 checkpoint；确认时复核来源固定范围、state 指纹、checkpoint、基线 hash，以及目标仍然缺失。若生成期间滚动文件被重新创建、修改为其它内容，或 state/来源分支变化，候选过期并拒绝覆盖。确认才原子创建文件、持久化 state/ledger，并刷新三份索引；写文件后进程退出但 state 未推进时，重复确认可按候选 hash 完成恢复。
