...
面板的推理桥接为每一轮创建一次性 runtime session，物化历史、执行一次 `sessions.send`，搬回新增 transcript 后注销并清理 runtime session。确定性测试覆盖这条普通对话流程；真实 runtime 的 bootstrap、工具与记忆状态仍须按 #48 复验。完整命令语义依赖 gateway 的会话和命令分派状态：
...
若把命令字符串送进普通消息接口，部分命令的状态会随临时 session 被删除，部分命令
会破坏增量 transcript 假设，也无法保证权限和返回结果正确。因此当前实现只提供下述
结构化命令范围，不提供透明命令透传。
...
- 这里的 `chat.send` 只记录固定 OpenClaw 版本的内部委托依据，不是面板可调用的 RPC；
  面板登记的发送方法始终只有 `sessions.send`。
...
- **安全事实（划重点）**：面板采用一条精确请求 `operator.read` / `operator.write` / `operator.admin` 的本机控制连接；`sessions.patch`、`sessions.compact`、`sessions.delete` 需要 admin。owner 判定 `senderIsOwnerByScope` 只要求「内部 channel + 连接持有 `operator.admin`」即成立。这意味着：**任何 `/` 文本一旦漏进推理桥接，`/bash`、`/restart` 这类命令会被 gateway 自动授权执行。** 当前服务端拒绝一切 `/` 开头的普通消息，这是必需的隔离防线。scope 精确握手与 RPC 允许列表只缩小意外调用面，不能替代这条内容隔离。

### 由此确立的两条设计原则

1. **永不把命令当 `/` 文本带内塞进推理桥接。** 普通消息发送路径继续以 `SLASH_COMMANDS_UNSUPPORTED` 拒绝 `/` 开头输入，命令永不从推理桥接意外执行。
...
| 类 | 命令（举例） | 面板怎么做 | 当前状态 |
|---|---|---|---|
| **A 面板原生** | `/model` `/think` `/reasoning` `/new` | 存进**面板会话 metadata**，每轮推理经 `sessions.create` 参数或 `sessions.patch` 应用到临时 session。`/new` 新建面板会话；会话中的模型切换也是这项 metadata 能力。 | ✅ |
| **B `/compact`（特殊）** | `/compact` | 面板会话无持久 gateway session 可压。面板物化当前有效分支，在临时 session 应用会话模型/thinking/reasoning 后调用 **`sessions.compact` typed RPC**（绝不发送命令文本），只采纳原 transcript 上“历史前缀完全不变且唯一新增”的合法 `compaction` entry，再原子提交到面板权威 transcript。完整历史继续显示；最新摘要、kept tail 与压缩后消息组成后续有效上下文。 | ✅ |
| **C 信息类**（只读代理，低风险） | `/help` `/commands` `/status` `/models` `/tools` `/usage` | 调已核实的只读 RPC/CLI，或由面板基于 allowlist / 权威 transcript 生成。数据来源未核实的命令不进入 allowlist。 | ✅ |
| **D gateway 管理 / owner 全局** | `/config` `/restart` `/mcp` `/plugins` `/reset` | 属于 gateway 管理面。面板是会话 UI，不是 gateway 控制台；`/reset` 对面板会话无对应语义。 | 默认不做 |

- 对**真实 active/reset 会话**执行任何命令，等同于向 OpenClaw 权威来源写入；它违反
  [`architecture.md`](../architecture.md) 的只读来源边界，不属于当前支持范围。
...
- 当前没有启用开关，也不进入 allowlist；任何 `/bash` 派发都拒绝。若将来立项，必须先独立确定执行模型、数值约束和部署开关。
...
- **需要独立安全设计（当前实现规格不包含该能力）**：进程执行还是 shell 文本、cwd、环境变量、超时、输出上限、进程终止、并发上限、审计日志，以及「开启要求登录已启用」的启动期校验。在这份安全设计落定前，`/bash` 不进入支持范围。

### 交互形态：打命令（type-to-invoke），不是点菜单

- 输入框敲 `/` 触发命令补全，而非纯下拉菜单。skill 命令是动态注册的
  (`skill:<name>`)，目录只用于发现和说明；可执行性仍由面板 allowlist 决定。
...
- 输入以 `/` 开头的内容：普通消息生成 run 在执行前以 `SLASH_COMMANDS_UNSUPPORTED`
  失败，不调用 gateway、不写入 panel transcript。上表已支持命令通过**独立命令派发
  路径**提供，不改变普通消息路径拒绝 `/` 的行为。
