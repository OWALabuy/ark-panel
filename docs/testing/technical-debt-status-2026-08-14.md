...
| #48 | Re-run each minimally identified target only after explicit per-runtime authorization, fixed-version confirmation and an exact channel-free/zero-bindings preflight; use fictional canary data and do not retain credentials, message bodies or unnecessary private paths. | The default-off harness is hardened at `ca95431`: it requires explicit config/sessions/workspace paths, an empty isolated sessions root, pinned identities, exact effective tools, one send, direct tool-result continuity and constrained cleanup. It has not been run live, so historical results are not promoted to current support. Follow the gated [runtime acceptance runbook](runtime-acceptance.md). |
