...
| --- | --- | --- |
| #21 | An explicitly authorized, zero-binding OpenClaw compaction run must show pre/post revision and usage, including reload. | The fixture covers the UI projection only; it intentionally cannot prove live compaction semantics. Use the gated [runtime acceptance runbook](runtime-acceptance.md), extended for this explicit compaction target rather than inferred from an ordinary browser run. |
