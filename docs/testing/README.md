...
本矩阵更新于 2026-08-22。既有行仍以各自行内 commit 为实现或证据快照；#41 的
...
| Firefox desktop/coarse-pointer automation | `current automated` | real Firefox/WebDriver against fictional local fixture；CI baseline | `npm run test:browser` | 2026-08-14 | `ada1548` | 不适用 | 当前 Firefox DOM、网络、SSE、桌面与 500px coarse-pointer fixture；含 #13 菜单、#27 synthetic ordered text/tool args、#19/#20 会话状态与设置恢复，以及 #50 内存 success screenshot 的精确 viewport/非空 PNG 检查；不等同实机触屏、人工视觉签字或 live runtime | [current browser automation](browser-acceptance.md) |
| External Markdown image consent and network boundary | `current automated` | frontend unit contracts + Firefox loopback request probe；日期化三轮 2/2 clean | `npm test && npm run test:browser` | 2026-08-14 | `6574548` | 不适用 | 未同意时不加载外部图片；仅显式跨主机链接可脱敏导航；同主机异端口仍拒绝；不证明其它浏览器或 live runtime | [browser evidence](browser-acceptance.md) |
...
| Capability | Status | Evidence kind | Command | Date | Panel commit | OpenClaw version | Scope | Link |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GitHub-hosted CI parse, push execution, job attribution, and clean-run result | `historical pass` | GitHub Actions `ubuntu-latest` push runs；local contract separately locks PR trigger/concurrency | [clean run](https://github.com/OWALabuy/ark-panel/actions/runs/32578060100) + [attributed failure](https://github.com/OWALabuy/ark-panel/actions/runs/32565380669) | 2026-08-22 | `5e0e610` | Node 22 | current-main clean run 的 baseline/coverage/deployment 三 job 全绿；故障 run 中 Firefox step 令 baseline/整次 workflow 失败，而 coverage/deployment 独立成功；#37 已关闭。未把未实跑的 PR/cancel 行为冒充 hosted evidence | [workflow](../../.github/workflows/ci.yml) |
...
| #43 current external-image Firefox repetitions | `historical pass` | real Firefox 153.0.1 + geckodriver 0.37.0，三轮完整 suite 2/2 clean | `npm run test:browser` | 2026-08-14 | `6574548` | 不适用 | 三轮均覆盖 loopback 零自动请求、显式 no-referrer/no-cookie/no-opener 导航与同主机异端口拒绝；不倒写前一日 partial | [browser evidence](browser-acceptance.md) |
...
| #27 bounded terminal tool-result rendering | `historical pass` | real Firefox 153.0.1 + geckodriver 0.37.0 against pinned `2026.6.11` result-shaped fixture | `npm test` + `npm run test:browser` | 2026-08-22 | `51abe13` | `2026.6.11` contract | 566/566 unit/integration tests and desktop/mobile 2/2；tool card displays bounded terminal JSON result in place；不声称 partial result 或 live model acceptance | [browser evidence](browser-acceptance.md) |
...
| #50 semantic CSS layers success screenshots | `historical pass` | real Firefox 153.0.1 + geckodriver 0.37.0；desktop/mobile in-memory PNG capture | `npm run test:browser` | 2026-08-14 | `ada1548` | 不适用 | canonical suite 2/2 clean；另在等价 topic diff 上三轮 2/2 clean、一次功能断言全过但 cleanup probe 失败；每次成功截图按实际 viewport 验证尺寸、颜色数与熵且不落盘，不替代人工审美或跨浏览器验收 | [browser evidence](browser-acceptance.md) |
| Live upstream stream sequence and tool-result event schema | `unknown` | privacy-safe collector exists；host execution blocked before dispatch enforcement | 显式 gate/path 环境 + `npm run --silent test:tool-result-schema-probe -- <fixed arguments>`；仅可在 exact-command 执行前强制或 disposable OS/container sandbox 中运行 | 未执行 | `f9f20d4` | 目标 `2026.6.11` | 零 bindings 与事后 args/tool-count 检查不能阻止错误命令先执行；2026-08-22 未实跑，不外推真实模型内容；当前 panel 只采用 pinned terminal `data.result` contract | [2026-08-22 boundary](live-probe-results-2026-08-22.md#tool-result-schema-probe-27) |
| Live compaction revision, usage, and fresh reload | `historical pass` | explicitly authorized isolated live probe | 显式 gate/四个 path 环境 + `npm run --silent test:compaction-live-probe -- <fixed arguments>` | 2026-08-22 | `28dfdf2` | `2026.6.11` | 固定可裁剪虚构历史完成一次真实 provider/compact、零 send、effective tools 精确空；verified entry pre total 59,875，fresh post total 163，同一 1,000,000 context window，revision 改变且 fresh reload 精确匹配；agent/root/listener/process 残留均为0 | [2026-08-22 evidence](live-probe-results-2026-08-22.md#compaction-revision-and-usage-21) |
...
| Live Gateway exact `operator.read/write/admin` authorization | `historical pass` | two explicitly authorized isolated live probes using the production control client | compaction/runtime probes with fixed gates and paths | 2026-08-22 | `f9f20d4` | `2026.6.11` | 两个零绑定临时 agent 均通过 exact three-scope hello 并到达 read/write/admin typed RPC；证明当前本机控制连接，不外推版本、remote 或新增 RPC | [2026-08-22 evidence](live-probe-results-2026-08-22.md#exact-three-scope-gateway-authorization-42) |
...
| Current `panel-claude-runtime` bootstrap/memory behavior | `historical pass` | explicitly authorized exact configured target, zero bindings | one fixed `openclaw agent --agent panel-claude-runtime --session-key … --json` turn + exact `sessions.delete` | 2026-08-22 | `1cceb76` | `2026.6.11` | Claude target injected 7/7 present bootstrap documents without truncation; one `memory_search`, zero tool failures, terminal completion; temporary session deleted | [2026-08-22 evidence](live-probe-results-2026-08-22.md#bootstrap-and-memory-continuity-48) |
| Current `panel-main-runtime` bootstrap/memory behavior | `historical pass` | explicitly authorized exact configured target, zero bindings | one fixed `openclaw agent --agent panel-main-runtime --session-key … --json` turn + exact `sessions.delete` | 2026-08-22 | `1cceb76` | `2026.6.11` | Named Gemini runtime injected all 6 bootstrap documents present in its reused main workspace; `MEMORY.md` was absent rather than silently claimed; one `memory_search`, zero tool failures; temporary session deleted | [2026-08-22 evidence](live-probe-results-2026-08-22.md#bootstrap-and-memory-continuity-48) |
| Current `paneltest` bootstrap/memory behavior | `historical pass` | explicitly authorized exact configured fixture target, zero bindings | one fixed `openclaw agent --agent paneltest --session-key … --json` turn + exact `sessions.delete` | 2026-08-22 | `1cceb76` | `2026.6.11` | Claude fixture injected 7/7 present bootstrap documents without truncation; one `memory_search`, zero tool failures, terminal completion; temporary session deleted | [2026-08-22 evidence](live-probe-results-2026-08-22.md#bootstrap-and-memory-continuity-48) |
| Disposable runtime probe bootstrap/memory behavior | `failed` | explicitly authorized isolated live model run | 显式 gate/三个 path 环境 + `npm run --silent test:runtime-acceptance -- <fixed arguments>` | 2026-08-22 | `a45b6ec` | `2026.6.11` | native transcript/cache 修订后的一次性零绑定 agent 通过 configured/effective tools，send 到达终态；同轮 Gateway 记录 embeddings quota failure，probe 返回 `PROBE_MEMORY_RESULT_INVALID`、cleanup null。agent/root 随后全部删除，不外推到 #48 配置 targets | [2026-08-22 evidence](live-probe-results-2026-08-22.md#bootstrap-and-memory-continuity-48) |
...
- [`live-probe-results-2026-08-22.md`](live-probe-results-2026-08-22.md)
