...
| Live compaction revision, usage, and fresh reload | `failed` | explicitly authorized isolated live probe | 显式 gate/四个 path 环境 + `npm run --silent test:compaction-live-probe -- <fixed arguments>` | 2026-08-22 | `8b6d35d` | `2026.6.11` | 来源确认复跑完成一次真实 provider/compact、零 send、effective tools 精确空且 constrained cleanup；raw compact accepted且entry通过bridge验证，但panel固定返回 `PROBE_PANEL_NO_EFFECTIVE_REDUCTION` 并保留原revision，#21仍缺 committed revision/reload | [2026-08-22 evidence](live-probe-results-2026-08-22.md#compaction-revision-and-usage-21) |
