...
| Current `panel-claude-runtime` bootstrap/memory behavior | `unknown` | current default-off runbook + superseded historical pass | 显式 gate/三个 path 环境 + `npm run --silent test:runtime-acceptance -- <fixed arguments>`（需逐目标授权） | 未执行 | `f9f20d4` | 目标 `2026.6.11` | 当前 probe 要求唯一 config target、零 bindings、仅 semantic-empty registry 的专用 sessions root、固定 fictional canary、精确 effective tool 与受限清理；尚未执行，不能沿用 2026-07-12 结果 | [runtime runbook](runtime-acceptance.md) |
| Current `panel-main-runtime` bootstrap/memory behavior | `unknown` | current default-off runbook + superseded historical failure | 显式 gate/三个 path 环境 + `npm run --silent test:runtime-acceptance -- <fixed arguments>`（需逐目标授权） | 未执行 | `f9f20d4` | 目标 `2026.6.11` | 当前 probe 的相同 fail-closed、semantic-empty registry 边界尚未在此目标执行；历史失败也不能推断当前结果 | [runtime runbook](runtime-acceptance.md) |
| Current `paneltest` bootstrap/memory behavior | `unknown` | current default-off runbook, no current result | 显式 gate/三个 path 环境 + `npm run --silent test:runtime-acceptance -- <fixed arguments>`（需逐目标授权） | 未执行 | `f9f20d4` | 目标 `2026.6.11` | 兼容目标仍须使用独占、仅 semantic-empty registry 的 sessions root 独立证明，不能从其它 runtime 推广 | [runtime runbook](runtime-acceptance.md) |
| Disposable runtime probe bootstrap/memory behavior | `failed` | explicitly authorized isolated live model run | 显式 gate/三个 path 环境 + `npm run --silent test:runtime-acceptance -- <fixed arguments>` | 2026-08-22 | `f9f20d4` | `2026.6.11` | 临时零绑定 agent 通过 configured/effective tools 并完成一次 send，但 required unique direct memory-result chain 未通过且产生 `skills-prompts` cleanup residual；agent/root 随后全部删除，不外推到 #48 配置 targets | [2026-08-22 evidence](live-probe-results-2026-08-22.md#bootstrap-and-memory-continuity-48) |
