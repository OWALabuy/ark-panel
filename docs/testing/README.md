...
| Live compaction revision, usage, and fresh reload | `failed` | explicitly authorized isolated live probe | 显式 gate/四个 path 环境 + `npm run --silent test:compaction-live-probe -- <fixed arguments>` | 2026-08-22 | `a928521` | `2026.6.11` | 最终复跑完成一次真实 provider/compact、零 send、effective tools 精确空且 constrained cleanup 完成；Panel API 返回 exact `NO_EFFECTIVE_REDUCTION` 并保留原 revision，但当前 envelope 不保留 upstream/panel reason 来源，#21 仍缺 accepted revision/reload | [2026-08-22 evidence](live-probe-results-2026-08-22.md#compaction-revision-and-usage-21) |
