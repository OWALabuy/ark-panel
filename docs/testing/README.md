...
| Live upstream stream sequence and tool-result event schema | `unknown` | privacy-safe collector + explicit live probe available；not executed | 显式 gate/path 环境 + `npm run --silent test:tool-result-schema-probe -- <fixed arguments>`（需明确目标与授权） | 未执行 | `35057f4` | 目标 `2026.6.11` | 探针固定一次 `exec`、要求零 bindings 与精确 effective tools；未实跑前 sequence/result shape 仍未知，且不替代 #48 | [stream probe runbook](streaming-acceptance.md#当前-tool-result-schema-探针尚未实机执行) |
| Live compaction revision, usage, and fresh reload | `unknown` | default-off privacy-safe probe available；not executed | 显式 gate/四个 path 环境 + `npm run --silent test:compaction-live-probe -- <fixed arguments>`（需明确目标与授权） | 未执行 | `86c75db` | 目标 `2026.6.11` | 固定虚构 transcript、一次 create/compact、零 send、专用空 workspace、fresh reload 与 constrained cleanup；离线测试不替代 live #21 evidence | [compaction runbook](compaction-acceptance.md) |
